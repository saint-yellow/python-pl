...
class Solution:
    ...
    def method1(self, node: TreeNode, depth: int) -> int:
        ...
        leftDepth = self.method1(node.left, depth+1)
        rightDepth = self.method1(node.right, depth+1)
    ...
    def method2(self, node: TreeNode) -> int:
        ...
        leftDepth = self.method2(node.left)
        rightDepth = self.method2(node.right)

        # 一棵二叉树的最大深度 = 子树的最小深度 + 1 
        # 1代表根节点的深度
