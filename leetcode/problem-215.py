...
class Solution:
    ...
    def method1(self, numbers: List[int], k: int) -> int:
        ...
        def quick_sort(numbers: List[int], left_pointer: int, right_pointer: int):
            if right_pointer - left_pointer <= 0:
                return
            pivot_pointer = partition(numbers, left_pointer, right_pointer)
            quick_sort(numbers, left_pointer, pivot_pointer-1)
            quick_sort(numbers, pivot_pointer+1, right_pointer)

        def partition(numbers: List[int], left_pointer: int, right_pointer: int) -> int:
            ...
            while True:
                ...
                if left_pointer >= right_pointer:
                    ...
                    swap(numbers, left_pointer, right_pointer)

            swap(numbers, left_pointer, pivot_pointer)

            return left_pointer

        def swap(numbers: List[int], pointer1: int, pointer2: int):
